...
Due to Java's constant factor, the below code may TLE on a couple of test cases.
...
public class EulerianCycle {
	public static void main(String[] args) throws IOException {
		...
		// Check that all degrees are even
		for (int d : degree) {
			if ((d & 1) == 1) {
		...
		while (!stack.isEmpty()) {
			int u = stack.peek();
			while (!g[u].isEmpty() && seen[g[u].get(g[u].size() - 1)[1]]) {
				g[u].remove(g[u].size() - 1);  // skip used edges
			}
			if (g[u].isEmpty()) {
				path.add(u);
				stack.pop();
			} else {
				int[] edge = g[u].remove(g[u].size() - 1);
